...
function doQuit(idx){
	let ref_this = $("ul.nav-tabs li a.active");

	if(ref_this.attr('id') == idx){
	 	let href = ref_this.attr('href').substring(1);
...
function doDestroyDatatable(tbl){
	let table = $('.'+tbl+'').DataTable();
	table.destroy();
}

function doSetDatatable(tbl){
  $('.'+tbl+'').DataTable({
    "destroy": false,
    "bPaginate": false,
    "bLengthChange": false,
...
function doSetDatatableWithFilter(tbl){
  $('.'+tbl+'').DataTable({
    "destroy": true,
    "bPaginate": true,
    "bLengthChange": false,
